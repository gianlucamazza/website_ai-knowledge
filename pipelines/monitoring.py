...
class PipelineMetrics:
    ...
    def __init__(self):
        ...
        self.duplicate_rate = Gauge(
            'pipeline_duplicate_rate',
            'Rate of duplicate articles detected'
        )
...
class PerformanceMonitor:
    ...
    def record_pipeline_run(self, status: str, trigger: str = 'manual', duration: Optional[float] = None):
        ...
        try:
            async with get_db_session() as session:
                ...
                recent_runs = await session.execute(
                    ...
                    ).order_by(PipelineRun.started_at.desc()).limit(10)
                )
...
class AlertManager:
    ...
    async def check_alerts(self, health_status: Dict[str, Any], performance_summary: Dict[str, Any]):
