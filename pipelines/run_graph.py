    ...
    async def _run_ingest():
        try:
            ...
            async with get_db_session() as session:
                ...
                results_table.add_row("Articles Discovered", str(results["total_articles_discovered"]))
...
@app.command()
def publish(
    article_ids: List[str] = typer.Option([], "--article", "-a", help="Specific article IDs to publish"),
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                with Progress(console=console) as progress:
                    task = progress.add_task("Publishing articles...", total=len(articles_to_publish))
...
@app.command()
def status(
    run_id: Optional[str] = typer.Option(None, "--run-id", "-r", help="Show status for specific run"),
                ...
                if run_id:
                    ...
                    if recent_runs:
                        ...
                        for run in recent_runs:
                            duration = "Running..." if not run.completed_at else f"{run.duration_seconds:.1f}s"
                            runs_table.add_row(
                                str(run.id)[:8],
                    ...
                    stats_query = select(
                        ...
                        func.count().filter(Article.status == ContentStatus.COMPLETED).label('completed'),
                        func.count().filter(Article.status == ContentStatus.PROCESSING).label('processing'),
                        func.count().filter(Article.status == ContentStatus.FAILED).label('failed'),
                        func.count().filter(Article.current_stage == PipelineStage.INGEST).label('ingest'),
                        func.count().filter(Article.current_stage == PipelineStage.NORMALIZE).label('normalize'),
                        func.count().filter(Article.current_stage == PipelineStage.DEDUP).label('dedup'),
                        func.count().filter(Article.current_stage == PipelineStage.ENRICH).label('enrich'),
                        func.count().filter(Article.current_stage == PipelineStage.PUBLISH).label('publish'),
                    ).select_from(Article)
                ...
                if list_all or sync:
                    ...
                    if sources_list:
                        ...
                        for source in sources_list:
                            last_crawl = source.last_crawl.strftime("%Y-%m-%d %H:%M") if source.last_crawl else "Never"
                            frequency = f"{source.crawl_frequency}s" if source.crawl_frequency else "N/A"
...
def display_pipeline_results(results: dict):
    ...
    if stage_results:
        ...
        for stage, stage_data in stage_results.items():
            if stage_data:
                details = ", ".join([f"{k}: {v}" for k, v in stage_data.items() if isinstance(v, (int, float, str))])
                stage_table.add_row(stage.title(), "✓", details[:50] + "..." if len(details) > 50 else details)
    ...
    if errors:
        ...
        for error in errors[-5:]:  # Show last 5 errors
            console.print(f"  - [{error.get('stage', 'unknown')}] {error.get('message', 'Unknown error')}")
...
def display_run_status(pipeline_run):
    ...
    panel = Panel(
        ...
        f"Duration: {pipeline_run.duration_seconds:.1f}s" if pipeline_run.duration_seconds else "Running...",
