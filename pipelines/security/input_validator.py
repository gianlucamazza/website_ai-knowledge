...
class InputValidator:
    ...
    def validate_html_content(self, content: str, field_name: str = "content") -> str:
        ...
        content = re.sub(r'<(iframe|embed|object)[^>]*>.*?</\1>', '', content, flags=re.DOTALL | re.IGNORECASE)
        ...
        def validate_dict_values(d: dict) -> dict:
            ...
            for key, value in d.items():
                if isinstance(value, str):
                    ...
                    result[key] = self.validate_string(value, f"{field_name}.{key}", max_length=10000)
                elif isinstance(value, dict):
                    result[key] = validate_dict_values(value)
                elif isinstance(value, list):
                    result[key] = [
                        validate_dict_values(item) if isinstance(item, dict) 
                        else self.validate_string(item, f"{field_name}.{key}[]", max_length=10000) if isinstance(item, str)
                        else item
                        for item in value
                    ]
                else:
    ...
    def validate_batch(self, items: List[Dict[str, Any]], validators: Dict[str, callable]) -> List[Dict[str, Any]]:
        """
        ...
        for i, item in enumerate(items):
            ...
            for field_name, validator_func in validators.items():
                if field_name in item:
                    try:
                        validated_item[field_name] = validator_func(item[field_name], f"item[{i}].{field_name}")
