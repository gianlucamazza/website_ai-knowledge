...
class SourceManager:
    ...
    def load_sources_config(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self.sources_config.get('sources', []))} source configurations")
        ...
        try:
            ...
            for source_config in self.sources_config.get('sources', []):
                ...
                if source_name in existing_sources:
                    ...
                    source.crawl_frequency = source_config.get('config', {}).get('crawl_frequency', 3600)
                    source.max_articles_per_run = source_config.get('config', {}).get('max_articles_per_run', 100)
                    ...
                    source = Source(
                        ...
                        crawl_frequency=source_config.get('config', {}).get('crawl_frequency', 3600),
                        max_articles_per_run=source_config.get('config', {}).get('max_articles_per_run', 100),
                    )
                    session.add(source)
        ...
        try:
            ...
            query = select(Source).where(
                ...
                (
                    ...
                    (Source.last_crawl + timedelta(seconds=Source.crawl_frequency) <= current_time)
                )
            )
    ...
    async def _should_ingest_article(self, article_data: Dict, source: Source, session: AsyncSession) -> bool:
    ...
    async def _create_article(self, article_data: Dict, source: Source, session: AsyncSession) -> None:
