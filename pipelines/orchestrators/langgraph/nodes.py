    ...
    async def normalize_content(self, state: PipelineState) -> PipelineState:
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                avg_quality_score = total_quality_score / processed_count if processed_count > 0 else 0.0
                ...
                for article_id in processed_articles:
                    try:
                        ...
                        for dup_id, similarity in lsh_duplicates:
                            if dup_id not in all_duplicates or similarity > all_duplicates[dup_id]["similarity"]:
                                all_duplicates[dup_id] = {
                                    "similarity": similarity,
                                    "method": "lsh"
                                }
                        ...
                        if all_duplicates:
                            ...
                            logger.info(f"Found {len(all_duplicates)} duplicates for article {article_id}")
    ...
    async def _build_dedup_indices(self, session: AsyncSession) -> None:
        ...
        try:
            ...
            query = select(Article).where(
                ...
                Article.current_stage.in_([PipelineStage.DEDUP, PipelineStage.ENRICH, PipelineStage.PUBLISH])
            )
            ...
            if existing_articles:
                logger.info(f"Building deduplication indices from {len(existing_articles)} existing articles")
