...
class MarkdownGenerator:
    ...
    def _generate_frontmatter(self, article: Article) -> Dict:
        ...
        frontmatter = {
            ...
            'pubDate': article.publish_date.isoformat() if article.publish_date else datetime.utcnow().isoformat(),
    ...
    def _determine_difficulty(self, article: Article) -> str:
        ...
        # Advanced indicators
        advanced_terms = [
            'algorithm', 'optimization', 'advanced', 'complex', 'sophisticated',
            'implementation', 'architecture', 'framework', 'system design'
        ]
        
        # Intermediate indicators
        intermediate_terms = [
            'configure', 'setup', 'install', 'deploy', 'integrate',
            'customize', 'extend', 'modify'
        ]
    ...
    async def update_taxonomies(self, session: AsyncSession) -> bool:
        ...
        try:
            ...
            logger.info(f"Updated taxonomies: {len(tags_data)} tags, {len(categories_data)} categories")
            return True
        ...
        try:
            ...
            if content.startswith('---\n'):
                ...
                if end_idx != -1:
                    ...
                    return (frontmatter.get('id') or 
                           frontmatter.get('source', {}).get('id') or
                           frontmatter.get('articleId'))
